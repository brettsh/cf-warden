...
| `COOLDOWN_SEC`        | Minimum seconds between mode switches (default: 300)           |
...
The two scoring divisors control how much each signal contributes to the total score. A score above `SCORE_TRIGGER` (default: 100) activates attack mode.

The default values are calibrated for an 8-core server with a baseline request rate around 125 req/min (99th percentile ~430 req/min):

```
LOAD_SCORE_DIVISOR=0.27   # load of 28 scores 103 — triggers alone on a severe spike
REQ_SCORE_DIVISOR=25      # 2500 req/min scores 100 — triggers alone on a flood
```

**Scaling rule:** if your server has roughly double the CPU cores or double the normal request rate compared to the above, halve the respective divisor. A lower divisor makes each unit of signal worth more points.

**Finding your numbers with DRY_RUN:** if you're unsure where to start, set `DRY_RUN=true` and let the daemon run for 24 hours. It will score every run and write to the log without making any Cloudflare API calls. Then check:

```bash
grep 'score=' /var/log/cf-warden.log
python3 /path/to/cf-warden/cf_warden.py status
```

Look at the scores during normal operation — they should sit comfortably below 100. If you have a known attack in the log history, those scores should exceed 100. Adjust the divisors until the gap is clear, then set `DRY_RUN=false`.

**Reducing false positives:** if cf-warden triggers too readily, increase `SCORE_CONFIRM_COUNT` from 1 to 2. The score must then exceed the threshold on two consecutive cron runs (two minutes in a row) before attack mode activates.
